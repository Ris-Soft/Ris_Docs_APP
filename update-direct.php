...
function generateMDIndex($directory, $parentPath = '', $isRoot = false) {
    ...
    foreach ($subdirectories as $dir) {
        $mdContent .= str_repeat("  ", count(explode("/", $parentPath)) - 1) . "- [$dir]({$parentPath}?article=" . urlencode($dir) . '/index' . ")\n";
        $mdContent .= generateMDIndex($directory . '/' . $dir, "{$parentPath}" . urlencode($dir) . '/');
    }
...
// 将MD索引文件内容保存到一个文件
file_put_contents($directoryPath.'directory-true.md', $mdIndex);

echo "目录生成完成，已保存到 ".$directoryPath."directory-true.md 文件。";
    } else {
